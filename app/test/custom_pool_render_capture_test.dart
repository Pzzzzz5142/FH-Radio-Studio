...
void main() {
  testWidgets(
    'capture custom pool route with Flutter render tree',
    (tester) async {
      ...
      await _captureCustomPool(
        tester,
        prefs: prefs,
        logicalSize: const Size(1365, 900),
        outputPath: p.join(outDir.path, 'custom_pool_regular.png'),
      );
      await _captureCustomPool(
        tester,
        prefs: prefs,
        logicalSize: const Size(1365, 1400),
        outputPath: p.join(outDir.path, 'custom_pool_full.png'),
      );
    },
    skip: Platform.environment['CAPTURE_CUSTOM_POOL'] != '1',
  );
}

Future<void> _captureCustomPool(
  WidgetTester tester, {
  ...
  expect(find.text('未分配 · 3'), findsNothing);
  expect(
    tester.getTopLeft(find.text('Zero Config')).dy,
    lessThan(tester.getTopLeft(find.text('Half Config')).dy),
  );
...
const _captureTracks = [
  ...
  PoolTrack(
    ...
    title: 'Ready One',
    artist: 'Local Artist',
    source: r'C:\music\ready-one.wav',
    durationSec: 201,
    bpm: 132,
    key: '待分析',
    configured: true,
  ...
  PoolTrack(
    ...
    title: 'Ready Two',
    artist: 'Local Artist',
    source: r'C:\music\ready-two.wav',
    durationSec: 198,
    bpm: 134,
    key: '待分析',
    configured: true,
