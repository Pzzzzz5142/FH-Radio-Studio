...
GitHub Actions release packaging is validation-first. Push a release branch that matches the app version, for example `release/v0.1.0-rc.1`, and `.github/workflows/release.yml` will run the Python and Flutter test suites, build the Windows release archive, generate a SHA256 file, and upload both files as a workflow artifact. The staging push does not create, update, or publish a GitHub Release. Download the `FHRadioStudio-v<version>-windows-x64` workflow artifact and test it before publishing.

After validation, publish from the GitHub-generated artifact rather than uploading files manually. Run the `Release` workflow manually with `action=publish-latest` and `release_ref=release/v<version>`, for example `release/v0.1.0-rc.1`. Optionally set `source_run_id` to the exact staged workflow run that was tested; otherwise the workflow uses the latest successful staged build on that release branch. The publish action downloads the staged artifact from GitHub, verifies its SHA256 file, creates the matching tag if needed, and publishes the GitHub Release as a prerelease when the version is an RC. If the app version contains build metadata such as `0.1.0-rc.1+7`, the workflow accepts either `v0.1.0-rc.1` or `v0.1.0-rc.1+7`. Normal RC builds show the release id only; build commit details are reserved for non-RC dev builds.
